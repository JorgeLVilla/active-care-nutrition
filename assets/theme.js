...
(function () {
    ...
    HeroAnimated.prototype = Object.assign({}, HeroAnimated.prototype, {
      initInterval: function () {
        ...
        var imagesHaveLoaded = this.allImages.every((element) => {
          return element
            .querySelector(".hero__image-wrapper")
  ...
  class AgeVerificationPopup extends HTMLElement {
    constructor() {
      ...
      if (
        (this.cookie || this.dataset.enabled === "false") &&
...
document.addEventListener("DOMContentLoaded", () => {
  let ingredientTabs = document.querySelectorAll(".ingredients__tab-content");
  let ingredientTabsBtns = document.querySelectorAll(".ingredients__tab-click");

  ingredientTabsBtns.forEach((tabBtn, id) => {
    tabBtn.addEventListener("click", (btn) => {
      ingredientTabs.forEach((tab) => {
        if (tab != ingredientTabs[id]) {
        ...
        setTimeout(()=>{
          ingredientTabs[id].classList.remove("hidden");
        }, 500);
        setTimeout(()=>{
          ingredientTabs[id].classList.add("open");
          document.querySelectorAll(".ingredients__ingredient-click").forEach((t)=>{
              t.classList.add("hidden");
              t.classList.remove("open");
          });
          document.querySelectorAll(".ingredients__ingredient-infos").forEach((t)=>{
              t.classList.add("hidden");
              t.classList.remove("open");
          });
          ingredientTabs[id].querySelector(".ingredients__ingredient-click").classList.remove("hidden");
          ingredientTabs[id].querySelector(".ingredients__ingredient-click").classList.add("open");
          ingredientTabs[id].querySelector(".ingredients__ingredient-infos").classList.remove("hidden");
          ingredientTabs[id].querySelector(".ingredients__ingredient-infos").classList.add("open");
        }, 600)
      });
    });
  });

  let ingredientClicks = document.querySelectorAll(".ingredients__ingredient-click");
  let ingredientDescriptions = document.querySelectorAll(".ingredients__ingredient-infos");

  ingredientClicks.forEach((btn, id)=>{
    btn.addEventListener("click", ()=>{
      ingredientClicks.forEach((click)=>{
        if(click != ingredientClicks[id]){
      ...
      ingredientDescriptions.forEach((desc)=>{
        if(desc != ingredientDescriptions[id]){
          desc.classList.remove("open");
        }
      });
      setTimeout(()=>{
        ingredientDescriptions[id].classList.add("open");
      }, 500);
    });
